...
SOURCE_SPACE = '../../gtfs/'
EXTENSION = '.txt'
TIMETABLE_HEADERS = ['from', 'to', 'departure', 'arrival', 'travel_time']
...
join_csv_on(
    ...
    WORKING_SPACE + 'stop_times_unsorted' + EXTENSION  # output file
)
exec_time.out()

print('Tri des heures d\'arrêts par voyage (trip_id) et heure de départs (departure_time)...')
csvsort(BASE_PATH + WORKING_SPACE + 'stop_times_unsorted' + EXTENSION,
        ['trip_id', 'departure_time'],
        output_filename=BASE_PATH + WORKING_SPACE + 'stop_times' + EXTENSION)
os.remove(BASE_PATH + WORKING_SPACE + 'stop_times_unsorted' + EXTENSION)
exec_time.out()

print('Génération des arcs de transports...')
arcs_transport = new_csv('arcs_transport', TIMETABLE_HEADERS)
stoptimes = open_csv('stop_times', 'SOURCE_SPACE')
last_two = Memo(2)
...
os.remove(BASE_PATH + WORKING_SPACE + 'stop_times' + EXTENSION)
exec_time.out()
...
for tr in transfers:
    ...
    while tr['from_stop_id'] == arc['from']:
        arc = {
            ...
            'arrival': int(arc['departure']) + int(tr['min_transfer_time']),
            'travel_time': tr['min_transfer_time']
        }
