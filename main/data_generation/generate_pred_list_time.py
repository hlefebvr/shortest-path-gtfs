...
class Graph:
    ...
    def add_link(self, x, y, heure_depart, duree_trajet):
        ...
        if x not in self._graph[y]:
            ...
            borne_inf_int, duree_trajet_int, delai_int, nb_trajet_int = self._graph[y][x][-1]; # recupere la derniere valeur
            
            delai_int_corr = delai_int if delai_int is not None else 0;
            borne_sup_int = add_minute( borne_inf_int, nb_trajet_int * ( delai_int_corr + duree_trajet_int ) )
...
for x in timetable:
    ...
    if memo.is_ready():
        ...
        if stop1['trip_id'] == stop2['trip_id']:
            if int(stop1['route_type']) in [1, 2, 3, 4, 5]:
                duree_trajet = diff_h( stop1['departure_time'], stop2['arrival_time'] );
                graph.add_link( stop1['stop_id'], stop2['stop_id'], stop1['departure_time'] , duree_trajet );
                i += 1;
                if i >= 200000:
                    ...
                    graph = Graph();
                    output_writer.close();
                    output_writer = open(BASE_PATH + './output/sub/sub_graph_'+ str(n_subgraph) +'.json', 'w');
