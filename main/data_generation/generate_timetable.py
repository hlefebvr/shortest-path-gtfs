...
def join_csv_on(file1, keys1, file2, keys2, output_file, sort_file1=True, sort_file2=True):
    # sorting file1
    csvsort(BASE_PATH + file1, keys1, output_filename=BASE_PATH +
            './tmp/file1.sorted', has_header=True)
    # sorting file2
    csvsort(BASE_PATH + file2, keys2, output_filename=BASE_PATH +
            './tmp/file2.sorted', has_header=True)
    # merging
...
join_csv_on('./tmp/stops_heart.csv',
            [0], '../../gtfs/stop_times.txt', [3], './tmp/timetable.csv')

print "Merging result with trips..."
join_csv_on('../../gtfs/trips.txt',
            [2], './tmp/timetable.csv', [10], './tmp/timetable2.csv')
os.remove(BASE_PATH + './tmp/timetable.csv')

print "Merging result with routes..."
join_csv_on('../../gtfs/routes.txt',
            [0], './tmp/timetable2.csv', [0], './tmp/timetable.csv')
os.remove(BASE_PATH + './tmp/timetable2.csv')

print "Sorting timetable by trip id and departure time"
csvsort(BASE_PATH + './tmp/timetable.csv', ['trip_id', 'departure_time'],
        output_filename=BASE_PATH + './output/timetable.csv', has_header=True)
os.remove(BASE_PATH + './tmp/timetable.csv')
