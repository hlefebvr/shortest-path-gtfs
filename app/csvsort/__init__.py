...
def csvsort(input_filename,
    ...
    with open(output_filename or input_filename, 'wt') as output_fp:
...
def csvsplit(reader, max_size):
    ...
    for row in reader:
        if writer is None:
            ntf = tempfile.NamedTemporaryFile(delete=False, mode='wt')
...
def memorysort(filename, columns):
    ...
    with open(filename, 'wt') as output_fp:
...
def mergesort(sorted_filenames, columns, nway=2):
    ...
    while len(sorted_filenames) > 1:
        ...
        with tempfile.NamedTemporaryFile(delete=False, mode="wt") as output_fp:
            writer = csv.writer(output_fp)
            merge_n += 1
